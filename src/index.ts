...
try {
	...
	updateUsage = limitsModule.updateUsage;
} catch {
...
async function main() {
	try {
		...
		// Get usage limits: prefer real data from payload, fallback to local estimation
		let usageLimits: { five_hour: any; seven_day: any };
		...
		if (input.rate_limits) {
			...
			if (updateUsage) {
				updateUsage(input.session_id, input.cost.total_cost_usd);
			}
		...
		if (usePayloadContext) {
			...
			if (current) {
				...
				// Use official percentage from Claude Code if available
				contextPercentage =
		...
		const data: StatuslineData = {
			...
			outputTokens: input.context_window?.total_output_tokens,
			exceeds200k: input.exceeds_200k_tokens ?? false,
			contextTokens,
			contextPercentage,
			...((input.rate_limits || getUsageLimits) && {
				usageLimits: {
